...
#in csv data is stored as quarternions x y z w then euler z y x and position x y z
test_df=pd.read_csv('../DataCollection-RED-Largedataset-Testing/OGP_dataset_collection_Rmatrix_RED.csv', names=['image_name', 'mat00', 'mat10', 'mat20','mat01','mat11','mat21','position_X', 'position_Y', 'position_Z'], header=None)
...
class SimTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        OGP_pose = self.OGP.iloc[idx, 1:9]
        ...
        OGP_pose[0,6]=(OGP_pose[0,6]-test_min_position_X)/(test_max_position_X-test_min_position_X)
        OGP_pose[0,7]=(OGP_pose[0,7]-test_min_position_Y)/(test_max_position_Y-test_min_position_Y)

        OGP_pose = OGP_pose.astype('float').flatten()
        #print('Normalized OGP: ',OGP_pose)
...
test_sim_dataset = SimTestDataset(csv_file='../DataCollection-RED-Largedataset-Testing/OGP_dataset_collection_Rmatrix_RED.csv',root_dir='../DataCollection-RED-Largedataset-Testing/')
...
class GraspEstimationModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc3 = nn.Linear(in_features=2048, out_features=8)
...
#writer = SummaryWriter('graphs_full/testingReal-0.001LR__time_{}'.format(timestamp))
...
model.load_state_dict(torch.load('OGP_saved_model_Full_0.001LR_Rmatrix.pth'))
#if torch.cuda.is_available():
    #model = model.cuda()
...
with torch.no_grad():
    for i, data in enumerate(testing_loader):
        ...
        for i in range(len(test_outputs)):
            #print('Test_labels: ',test_labels[i][:-2].numpy(),(test_labels[i][6].numpy()*(test_max_position_X-test_min_position_X))+test_min_position_X   ,    (test_labels[i][7].numpy()*(test_max_position_Y-test_min_position_Y))+test_min_position_Y)
            output_array=test_outputs[i].numpy()
            #print(np.array([output_array[0],output_array[1], output_array[2]]))
            binormal=np.array([output_array[0],output_array[1], output_array[2]])
            normal=np.array([output_array[3],output_array[4], output_array[5]])
            approach = np.cross(binormal,normal )
            r = R.from_matrix([binormal,normal,approach])
            #print(r.as_matrix())
            #saved quarternions are x y z w
            quaternions=r.as_quat()
            #r1=R.from_quat(quaternions)
            #print(r1.as_matrix())
            #saved file name,quarternions are x y z w,X,Y,Z
            rows=['image_name',quaternions[0],quaternions[1],quaternions[2],quaternions[3],   (output_array[6]*(test_max_position_X-test_min_position_X))+test_min_position_X   ,    (output_array[7]*(test_max_position_Y-test_min_position_Y))+test_min_position_Y  ,'Z_depth']
            print(rows)
            with open('../DataCollection-RED-Largedataset-Testing/OGP_dataset_collection_Rmatrix_Outputs.csv', 'a') as csvfile:
        ...
        #writer.add_scalars('Testing Loss w.r.t batch', { 'Testing' : test__loss/(i+1) },i+1)
        #writer.flush()
